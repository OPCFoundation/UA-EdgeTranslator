            ...
            try
            {
                ...
                switch (action)
                {
                    case "Authorize":
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints.ContainsKey(cpId))
                        {
                            OCPPCentralSystemAsset.ChargePoints.TryAdd(cpId, new ChargePoint());
                            OCPPCentralSystemAsset.ChargePoints[cpId].ID = cpId;
                        }
                    ...
                    case "MeterValues":
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.ContainsKey(meterValuesRequest.ConnectorId))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.TryAdd(meterValuesRequest.ConnectorId, new Connector(meterValuesRequest.ConnectorId));
                        }
                        ...
                        foreach (MeterValue meterValue in meterValuesRequest.MeterValue)
                        {
                            foreach (SampledValue sampledValue in meterValue.SampledValue)
                            {
                                ...
                                if (int.TryParse(sampledValue.Value, out int parsedInt))
                                {
                                    ...
                                    OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[meterValuesRequest.ConnectorId].MeterReadings.Add(reading);

                                    if (OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[meterValuesRequest.ConnectorId].MeterReadings.Count > 10)
                                    {
                                        OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[meterValuesRequest.ConnectorId].MeterReadings.RemoveAt(0);
                                    }
                                }
                            }
                        }
                    ...
                    case "StartTransaction":
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.ContainsKey(startTransactionRequest.ConnectorId))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.TryAdd(startTransactionRequest.ConnectorId, new Connector(startTransactionRequest.ConnectorId));
                        }
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[startTransactionRequest.ConnectorId].CurrentTransactions.ContainsKey(_transactionNumber))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[startTransactionRequest.ConnectorId].CurrentTransactions.TryAdd(_transactionNumber, transaction);
                        }

                        // housekeeping: Remove transactions that are older than 1 day
                        KeyValuePair<int, ChargePointTransaction>[] transactionsArray = OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[startTransactionRequest.ConnectorId].CurrentTransactions.ToArray();
                        for (int i = 0; i < transactionsArray.Length; i++)
                        {
                            ...
                            {
                                OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[startTransactionRequest.ConnectorId].CurrentTransactions.TryRemove(transactionsArray[i].Key, out _);
                            }
                        }
                    ...
                    case "StopTransaction":
                        ...
                        KeyValuePair<int, Connector>[] connectorArray = OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.ToArray();
                        for (int i = 0; i < connectorArray.Length; i++)
                        {
                            if (OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[connectorArray[i].Key].CurrentTransactions.ContainsKey(stopTransactionRequest.TransactionId))
                            {
                                OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[connectorArray[i].Key].CurrentTransactions[stopTransactionRequest.TransactionId].MeterValueFinish = stopTransactionRequest.MeterStop;
                                OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[connectorArray[i].Key].CurrentTransactions[stopTransactionRequest.TransactionId].StopTime = stopTransactionRequest.Timestamp;
                                break;
                            }
                        }
                    ...
                    case "StatusNotification":
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.ContainsKey(statusNotificationRequest.ConnectorId))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.TryAdd(statusNotificationRequest.ConnectorId, new Connector(statusNotificationRequest.ConnectorId));
                        }

                        OCPPCentralSystemAsset.ChargePoints[cpId].ID = cpId;
                        OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[statusNotificationRequest.ConnectorId].Status = statusNotificationRequest.Status.ToString();
