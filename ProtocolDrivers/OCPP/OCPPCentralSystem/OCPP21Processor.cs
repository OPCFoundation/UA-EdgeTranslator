            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "MeterValues":
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.ContainsKey(meterValuesRequest.EvseId))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.TryAdd(meterValuesRequest.EvseId, new Connector(meterValuesRequest.EvseId));
                        }
                        ...
                        foreach (MeterValue21 meterValue in meterValuesRequest.MeterValue)
                        {
                            foreach (SampledValue21 sampledValue in meterValue.SampledValue)
                            {
                                ...
                                OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[meterValuesRequest.EvseId].MeterReadings.Add(reading);

                                if (OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[meterValuesRequest.EvseId].MeterReadings.Count > 10)
                                {
                                    OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[meterValuesRequest.EvseId].MeterReadings.RemoveAt(0);
                                }
                            }
                        }
                    ...
                    case "TransactionEvent":
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.ContainsKey(transactionEventRequest.Evse.ConnectorId))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors.TryAdd(transactionEventRequest.Evse.ConnectorId, new Connector(transactionEventRequest.Evse.ConnectorId));
                        }
                        ...
                        if (!OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[transactionEventRequest.Evse.ConnectorId].CurrentTransactions.ContainsKey(transactionEventRequest.TransactionInfo.TransactionId.GetHashCode()))
                        {
                            OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[transactionEventRequest.Evse.ConnectorId].CurrentTransactions.TryAdd(transactionEventRequest.TransactionInfo.TransactionId.GetHashCode(), transaction);
                        }

                        // housekeeping: Remove transactions that are older than 1 day
                        KeyValuePair<int, ChargePointTransaction>[] transactionsArray = OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[transactionEventRequest.Evse.ConnectorId].CurrentTransactions.ToArray();
                        for (int i = 0; i < transactionsArray.Length; i++)
                        {
                            ...
                            {
                                OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[transactionEventRequest.Evse.ConnectorId].CurrentTransactions.TryRemove(transactionsArray[i].Key, out _);
                            }
                        }
                    ...
                    case "StatusNotification":
                        ...
                        OCPPCentralSystemAsset.ChargePoints[cpId].ID = cpId;
                        OCPPCentralSystemAsset.ChargePoints[cpId].Connectors[statusNotificationRequest.ConnectorId].Status = statusNotificationRequest.ConnectorStatus.ToString();
