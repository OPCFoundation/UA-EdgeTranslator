...
>
> The binding also **withdrew** its pre-0.3 terms. These are rejected with a message naming the replacement, rather than silently treated as their successor, because the replacements are not plain renames: `lorav:type` → `lorav:wireType`, `lorav:offset` → `lorav:addend`, `lorav:length` → `lorav:byteLength`, `lorav:enum` → `lorav:valueMap`, `lorav:presenceField`/`lorav:presenceBit` → `lorav:presentWhen` with field/bit, `lorav:switchField`/`lorav:switchValue` → `lorav:presentWhen` with field/value, `lorav:var` → `lorav:alias`, `lorav:ref`/`lorav:polynomial`/`lorav:compute`/`lorav:guard`/`lorav:transform` → keys inside `lorav:derived`, `lorav:validRange` → TD core `minimum`/`maximum`, `lorav:unece` → TD core `unit`, `lorav:brand`/`lorav:model` → `schema:manufacturer`/`schema:mpn`, and `lorav:hardwareVersion`/`lorav:softwareVersion` → `schema:version`/`schema:softwareVersion`.
>
> The supported terms are `lorav:byteOffset`, `lorav:byteLength`, `lorav:wireType`, `lorav:endian`, `lorav:bitmask`, `lorav:multiplier`, `lorav:divisor`, `lorav:addend`, `lorav:fPort`, `lorav:tag`, `lorav:alias`, `lorav:presentWhen` and `lorav:valueMap` at form level, plus `lorav:payloadLayout`, `lorav:tagFields`, `lorav:devEUI`, `lorav:joinEUI`, `lorav:macVersion`, `lorav:region`, `lorav:frequencyPlan`, `schema:manufacturer`, `schema:mpn`, `schema:version` and `schema:softwareVersion` at thing level. `lorav:wireType` accepts both the `xsd:` aliases and the native spellings (`u8`, `s16`, `u16`, …) the specification's examples use.
>
> A gated field whose `lorav:presentWhen` condition is not met reads as `null` rather than zero, so "absent from this uplink" stays distinguishable from "present and zero".
>
> For the `tlv` and `ctv` layouts, a value is located by scanning the uplink for its `lorav:tag` bytes rather than at a fixed offset, so a device may omit or reorder measurements between uplinks. Declare the tag composition once at thing level with `lorav:tagFields` and give each form its `lorav:tag`, as [Samples/MilesightEM300-TH.td.jsonld](Samples/MilesightEM300-TH.td.jsonld) does. A tag whose length disagrees with `lorav:tagFields` is rejected, since it would otherwise never match and the field would silently never appear.
...
> **Note**: LoRaWAN Gateway router configurations are plain JSON, not Thing Descriptions. A Basic Station `router_config` message describes the gateway's radio hardware — region, frequency range, data rates and SX1301 channel plan — and has no WoT affordances, so it is not modelled as an asset. Place the `router_config` JSON in the `settings` folder, named either after the gateway's DevEUI (`2CCF67FFFEFB3C8F.json`, 16 hex digits) or after the hardware model (`SX1301EU.json`), the latter serving as the default for any gateway without its own file. See [Samples/SX1301EU.json](Samples/SX1301EU.json) for an EU868 SX1301 example. These files are loaded when the network server starts, so a gateway is answered as soon as it sends its `version` message, without needing an asset to be onboarded first. Any other JSON in the `settings` folder is ignored: files are identified by their `"msgtype": "router_config"` field.

> **Note**: The gateway must run [LoRa Basics Station](https://github.com/lorabasics/basicstation), **not** the legacy Semtech UDP packet forwarder. UA Edge Translator implements the Basics Station LNS protocol over WebSockets; the UDP forwarder's `PUSH_DATA`/`PULL_DATA` datagrams are a different protocol on a different transport and are silently ignored. Build Basics Station for the matching concentrator (`platform=corecell` for SX1302, `platform=rpi` for SX1301) and point `tc.uri` at `ws://<host>:5000` (plaintext, with `DISABLE_TLS` set) or `wss://<host>:5001` (TLS). Note that the gateway's `station.conf` does not carry the radio plan: the channel plan is supplied by this server at runtime through the `router_config` message described above.
